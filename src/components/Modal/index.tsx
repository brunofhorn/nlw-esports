...
export function Modal({ title, close, centered, children }: IModal) {
  const { isPageLoading } = useContext(GameContext);

  const transitions = useTransition(isPageLoading, {
    from: { opacity: 0 },
    enter: { opacity: 1 },
    leave: { opacity: 0 },
    config: config.stiff,
          ...
          <>
            ...
            >
              <animated.div style={styles}>
                ...
                <Dialog.Title
                  className={`text-2xl font-black ${centered && 'text-center'}`}
                >
                  {title}
                </Dialog.Title>
                {centered ? (
